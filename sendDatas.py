...
#run - exec(open('sendDatas.py').read())
...
def send_mysql(msg_vector):
    ...
    try:
        with connection.cursor() as cursor:        
            sql = "INSERT INTO api_dataarduino (vazao, date) VALUES(%s, %s)"
            # Executa query Insert
            cursor.execute(sql,( msg_vector[0], msg_vector[1]))
        # Confirma a transação realizada
        connection.commit()
        print("Dados inseridos: vazao = "+str(msg_vector[0])+", date = "+str(msg_vector[1]))
    except:
...
def on_message(client, userdata, msg):
    ...
    calculoDaVazao = round(dict_json["vazao"], 2)
    ...
    msg_formated = [calculoDaVazao, Hour]
