...
const int pinoRele = GPIO_NUM_33; //PINO DIGITAL UTILIZADO PELO MÓDULO RELÉ
const int pinoBotao = GPIO_NUM_32; //PINO DIGITAL UTILIZADO PELO BOTÃO
...
void setup()
{
  ...
  //DEFINE O PINO COMO ENTRADA / "_PULLUP" É PARA ATIVAR O RESISTOR INTERNO DO ARDUINO PARA GARANTIR QUE NÃO EXISTA FLUTUAÇÃO ENTRE 0 (LOW) E 1 (HIGH)  
...
void loop() {
  ...
  // Publica mensagem MQTT
    ...
    last_time = millis();
     ...
     if((digitalRead(pinoBotao) == LOW) && (statusSolen == 0)){ 

//LIGA O MÓDULO RELÉ (LÂMPADA ACENDE)
...
//SENÃO, FAZ
      if((digitalRead(pinoBotao) == LOW) && (statusSolen == 1)){ 

//SE A LEITURA DO PINO FOR IGUAL A LOW E
